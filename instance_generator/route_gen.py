...
def truncated_norm(minimum, maximum, mean, sd):
    ...
    return (
        truncnorm(a=(minimum - mean) / sd, b=(maximum - mean) / sd, scale=sd, loc=mean)
        .rvs(size=1)
        .round()
        .astype(int)[0]
    )
...
def instance_generator(
    nb_aircraft=default_nb_aircraft,
    ...
    second_last_maintenance["seven_day"] = [
        truncated_norm(
            0,
            SECONDS_seven_dayS - (SECONDS_seven_dayS / 7),
            SECONDS_seven_dayS / 2,
            SECONDS_seven_dayS / 7,
        )
        for i in range(nb_aircraft)
    ]
    second_initial_counter["seven_day"] = [
        count for count in second_last_maintenance["seven_day"]
    ]
    ...
    AIRPORTS_MAINTENANCE["seven_day"] = np.random.randint(
        nb_airport, size=nb_airport_maintenance
    )
    ...
    for aircraft in range(nb_aircraft):
        ...
        if verbose:
            print(
                "For aircraft {} we have {} flights".format(
        ...
        for maintenance in second_last_maintenance:
            current_second_last_maintenance[maintenance] = second_last_maintenance[
                maintenance
            ][aircraft]

        if verbose:
            print(
                "For aircraft {} the current counter is {} for a maximum of {}".format(
                    aircraft,
                    current_second_last_maintenance["seven_day"],
                    flights_per_aircraft[aircraft],
                )
            )
        ...
        for i in range(flights_per_aircraft[aircraft]):
            ...
            if first_flight:
                if long and not short and aircraft == nb_aircraft - 1:
                    ...
                    if verbose:
                        print(
                            "First flight of aircraft {}, is {} - {}".format(
                ...
                key = (
                    str(flight_object.start_airport)
                    + "-"
                    + str(flight_object.end_airport)
                )
                if key not in flights_created:
                    ...
                    flight_object.end_date = (
                        flight_object.start_date + flight_object.length_fly
                    )
                ...
                usage_aircraft = (
                    current_second_last_maintenance["seven_day"]
                    / limit_second_last_maintenance["seven_day"]
                )
                ...
                if (
                    usage_aircraft > 0.5
                    and probability_add_maintenance >= 1.0
                    or usage_aircraft > 0.9
                ):
                    ...
                    # up being the same
                    all_airports_except_start = [
                        x
                        for x in AIRPORTS_MAINTENANCE["seven_day"]
                        if x != previous.start_airport
                    ]
                    ...
                    key = (
                        str(previous_flight.start_airport)
                        + "-"
                        + str(previous_flight.end_airport)
                    )
                    if key not in flights_created:
                        ...
                        previous_flight.end_date = (
                            previous_flight.start_date + previous_flight.length_fly
                        )
...
def main():
    ...
    parser.add_argument(
        "--aircraft", type=int, help="the number of aircraft in the outputted instance"
    )
    parser.add_argument(
        "--airport", type=int, help="the number of airports in the outputted instance"
    )
    parser.add_argument(
        "--meanFlightLength",
        type=int,
    ...
    parser.add_argument(
        "--minFlightLength", type=int, help="Minimum length of the flights (in minutes)"
    )
    parser.add_argument(
        "--maxFlightLength", type=int, help="Maximum length of the flights (in minutes)"
    )

    parser.add_argument(
        "--meanFlightAircraft", type=int, help="Average flights per aircraft"
    )
    parser.add_argument(
        "--varFlightAircraft", type=int, help="Variance of flights per aircraft"
    )
    parser.add_argument(
        "--minFlightAircraft", type=int, help="Minimum flights per aircraft"
    )
    parser.add_argument(
        "--maxFlightAircraft", type=int, help="Maximum flights per aircraft"
    )

    parser.add_argument(
        "--meanTat", type=int, help="Average TAT between two flights (in minutes)"
    )
    parser.add_argument(
        "--varTat", type=int, help="Variance TAT between two flights (in minutes)"
    )
    parser.add_argument(
        "--minTat", type=int, help="Minimum TAT between two flights (in minutes)"
    )
    parser.add_argument(
        "--maxTat", type=int, help="Maximum TAT between two flights (in minutes)"
    )
    ...
    parser.add_argument(
        "--output_file", type=str, help="The name of the output file of the instance"
    )
    ...
    parser.add_argument(
        "--gannt", action="store_true", help="This will output the gannt"
    )
    ...
    if args.default:
        ...
        if (
            args.meanFlightLength is not None
            ...
            var_length_flight = int(
                input("Variance of the length of the flights (still in minutes) : ")
            )
        ...
        if (
            args.meanFlightAircraft is not None
            and args.varFlightAircraft is not None
            and args.minFlightAircraft is not None
            and args.maxFlightAircraft is not None
        ):
            ...
            var_flight_per_aircraft = int(input("Variance flight per aircraft : "))
        ...
        if (
            args.meanTat is not None
            and args.varTat is not None
            and args.minTat is not None
            and args.maxTat is not None
        ):
        ...
        if (
            args.meanGroundTime is not None
            and args.varGroundTime is not None
            and args.minGroundTime is not None
            ...
            mean_on_ground = int(
                input("Mean time on ground between two flights (in minutes) : ")
            )
            var_on_ground = int(
                input("Variance time on ground between two flights (in minutes) : ")
            )
            min_on_ground = int(
                input("Min time on ground between two flights (in minutes) : ")
            )
            max_on_ground = int(
                input("Max time on ground between two flights (in minutes) : ")
            )
        solution, _ = instance_generator(
            ...
            min_length_flight,
            max_length_flight,
            mean_flight_per_aircraft,
            var_flight_per_aircraft,
            min_flight_per_aircraft,
            max_flight_per_aircraft,
            mean_tat,
            var_tat,
            min_tat,
            max_tat,
...
def gannt(solution):
    ...
    for flight in solution.flights:
        ...
        if (
            isinstance(flight, models.Maintenance)
            or flight.start_airport == flight.end_airport
        ):
            dict_flight["Resource"] = "Maintenance"
        else:
            dict_flight["Resource"] = (
                str(flight.start_airport) + " - " + str(flight.end_airport)
            )
    ...
    colors = [
        ...
        for i in range(len(solution.unique_flights) + 1)
    ]
    ...
    for flight in solution.flights:
        tmp.append(
            dict(
                x=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(flight.start_date)),
        ...
        tmp.append(
            dict(
                x=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(flight.end_date)),
                y=solution.nb_aircraft - flight.assigned_aircraft - 1,
    ...
    for flight in solution.flights:
        fig.add_trace(
            go.Scatter(
                x=[
                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(flight.end_date)),
                    time.strftime(
                        "%Y-%m-%d %H:%M:%S",
...
def asp_input_fact(output_file, solution):
    ...
    file = open("../instances/" + output_file + ".lp", "w+")
