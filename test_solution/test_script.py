...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir, 'instance_generator/models')))
...
def gantt_solution(instance, solution):
    ...
    print("number flights  {}".format(number_flights))
...
def main():
    ...
    for answer in solutions:
        ...
        file.close()
        if args.gantt:
            instances_sol = clyngor.solve(files=[instance])
